...
class GMEEK():
    ...
    def createPostHtml(self,issue):
        ...
        postBase["repoName"]=self.options.repo_name  # 修复：使用 self.options 而非全局变量
    ...
    def createFeedXml(self):
        ...
        feed.description(self.blogBase.get("subTitle", self.blogBase["subtitle"]))  # 兼容 subtitle 字段
    ...
    def addOnePostJson(self,issue):
        if len(issue.labels)>=1:
            ...
            self.blogBase[listJsonName][postNum]["postSourceUrl"]="https://github.com/"+self.options.repo_name+"/issues/"+str(issue.number)  # 修复：使用 self.options
